...
def upgrade() -> None:
    ...
    op.execute(users.update().where(users.c.email_verified.is_(None)).values(email_verified=sa.false()))
    op.alter_column('users', 'email_verified', existing_type=sa.Boolean(), nullable=False, server_default=None)
