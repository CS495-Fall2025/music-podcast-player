...
@patch("rss_music_db_service.playlists.update.make_session")
def test_update_playlist_returns_none_if_not_found_or_unauthorized(
    mock_make_session,
) -> None:
    mock_session = MagicMock()
    mock_make_session.return_value.__enter__.return_value = mock_session
...
@patch("rss_music_db_service.playlists.update.make_session")
def test_update_playlist_title_only(mock_make_session) -> None:
    mock_session = MagicMock()
    mock_make_session.return_value.__enter__.return_value = mock_session
    existing = Playlist(id=5, title="Old", description="Keep Me", created_by_user_id=1)
    mock_session.query.return_value.filter_by.return_value.first.return_value = existing

    result = update.update_playlist(playlist_id=5, user_id=1, title="New Only")

    assert result.title == "New Only"
