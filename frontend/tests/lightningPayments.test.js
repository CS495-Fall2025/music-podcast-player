...
describe("makeStreamValueMeta", () => {
  it("uses recipient information", () => {
    const meta = makeStreamValueMeta(50, [
      {
    ...
    expect(meta[0].customRecord).toStrictEqual({
      300: "recipient-1-custom-value",
    });
    expect(meta[0].meta.name).toBe("recipient-1");
  });

  it("divides sats randomly by split", () => {
    const meta = makeStreamValueMeta(5000, [
      {
        address: "recipient-1-addr",
        customKey: "300",
        customRecord: { 300: "recipient-1-custom-value" },
    ...
    expect(meta[0].meta.value_msat).toBeGreaterThan(
      meta[1].meta.value_msat * 5,
    );
